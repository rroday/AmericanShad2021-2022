...
colpal <- c("Atlantic" = "#4477AA",  "Bay" = "#FD5C70", "Brandywine 1" = "#228833", 
            "Brandywine 2" = "#AA3377", "Brandywine 3" = "#66CCEE",  "River" = "#E69F00",
...
dtc_final1.9 <- dtc_final1.8 %>%
 distinct(detection_timestamp_utc, animal_id, .keep_all = T) 

dtc_final2 <- dtc_final1.9 %>%
  filter(!Insertion_type == "Gastric") %>%
  filter(!Year == 2023)
...
#Subset to exclude 2 gastric tags
fsh2 <- fsh %>%
  filter(!Insertion_type == "Gastric")

# Average Fork Length per year 
fsh2 %>%
...
shapiro.test(fsh2$Fork.mm[fsh$Deploy.Year==2021])
shapiro.test(fsh2$Fork.mm[fsh$Deploy.Year==2022])
# Test for differences in variance across years     # Variances are equal too. 
levene.test(fsh2$Fork.mm, group = fsh2$Deploy.Year)
# Time for a t test to test in length differences across years
# No difference in length between years :)
t.test(fsh2$Fork.mm[fsh2$Deploy.Year==2021], fsh2$Fork.mm[fsh2$Deploy.Year==2022])
...
ggsave("Figure2.png", plot = last_plot(), width = 3, height = 3,
       units = "in", dpi = 500, path = "C:/Users/RER/Documents/Masters UD/Roday et al. 2024 Manuscript/FinalFigs")
...
ggsave("Figure3.jpeg", plot = last_plot(), width = 12, height = 7, units = "in",
       dpi = 500,  path = "C:/Users/RER/Documents/Masters UD/Roday et al. 2024 Manuscript/FinalFigs")
...
ggsave("Figure4.jpeg", plot = last_plot(), width = 12, height = 7, units = "in",
       dpi = 500,  path = "C:/Users/RER/Documents/Masters UD/Roday et al. 2024 Manuscript/FinalFigs")
...
FishDetects <- detection_events(residency_detects, location_col = "Region2", time_sep = Inf, condense = TRUE) 
...
ggsave("Figure5B.jpeg", plot = last_plot(), width = 7,  height =4,
       units = "in", dpi = 500,  path = "C:/Users/RER/Documents/Masters UD/Roday et al. 2024 Manuscript/FinalFigs")
...
FishDets.Time <- FishDetects2 %>% mutate (res_time_min = res_time_sec/60, 
...
FishDets.Time %>%
  ...
  ungroup() %>%
  group_by(location) %>%
  summarise(prop.time = sum(total.time)/length(animal_id),
            prop.time.wks = prop.time/604800,
            prop.time.day = prop.time/86400)
...
ggsave("Figure5A.jpeg", plot = last_plot(), width = 7,  height =4,
       units = "in", dpi = 500,  path = "C:/Users/RER/Documents/Masters UD/Roday et al. 2024 Manuscript/FinalFigs")
...
occupancy.avg %>%
  group_by(Year) %>%
  summarise(prop.time = (avg.time/sum(avg.time))*100)

Prop.occupancy.avg <- occupancy2 %>%
...
ggsave("Figure6.jpeg", plot = last_plot(), width = 7,  height = 4,
       units = "in", dpi = 500,  path = "C:/Users/RER/Documents/Masters UD/Roday et al. 2024 Manuscript/FinalFigs") 
...
occ2022 <- occupancy.avg2022 %>% ungroup %>% dplyr::select(location, total.time)
mat1 <- data.matrix(occ2022)
friedman.test(mat1)

occ2021 <- occupancy.avg2021 %>% ungroup %>% dplyr::select(location, total.time)
...
dtc_final1.6 %>%
  ...
  filter(animal_id %in% c(11,23,17,19)) %>%
  filter(!Receiver == "PROJ162") %>%
  mutate(detection_timestamp_utc = as.POSIXct(detection_timestamp_utc)) %>%
...
ggsave("Supplemental1.png", plot = last_plot(), width = 8, height = 5,
       units = "in", dpi = 300, path = "C:/Users/RER/Documents/Masters UD/Roday et al. 2024 Manuscript/FinalFigs")
...
ggsave("Supplemental2.png", plot = last_plot(), width = 8, height = 5,
       units = "in", dpi = 300, path = "C:/Users/RER/Documents/Masters UD/Roday et al. 2024 Manuscript/FinalFigs")
...
dtc_final2 %>%
  filter(!Year == 2023) %>%
  ggplot(aes(x = as.Date(detection_timestamp_utc), y = as.character(animal_id), color = Region, alpha = .2))+
  ...
  theme(panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank(),
        ...
        axis.title = element_text(size = 15)) +
  guides(alpha = "none", color=guide_legend(title="Region")) +
  labs(title = "", y= "Animal ID", x="Date")+
  scale_color_manual("Region",values=colpal, labels = c("Atlantic Ocean", "Delaware Bay", "Brandywine River 1", "Brandywine River 2", "Brandywine River 3", "C&D Canal", "Delaware River")) 

ggsave("Supplemental3.png", plot = last_plot(), width = 10,  height = 6,
       units = "in", dpi = 500,  path = "C:/Users/RER/Documents/Masters UD/Roday et al. 2024 Manuscript/FinalFigs") 
...
ggsave("Supplemental4.jpeg", plot = last_plot(), width = 10,height = 4, units = "in",
...
movement2.2 <- movement2 %>%
  filter(!animal_id %in% c(11,17,23,13, 14,19)) %>%
  pivot_longer(!animal_id, names_to = "Variable", values_to = "Value") %>%
...
movement2.1 %>% filter(!animal_id %in% c(13,14)) %>%
...
ggsave("Supplemental5A.png", plot = last_plot(), width = 6,height = 5, units = "in", dpi = 300,
       path = "C:/Users/RER/Documents/Masters UD/Roday et al. 2024 Manuscript/FinalFigs")
...
ggsave("Supplemental5B.png", plot = last_plot(), width = 6,height = 5, units = "in", dpi = 300,
       path = "C:/Users/RER/Documents/Masters UD/Roday et al. 2024 Manuscript/FinalFigs")
####   Figure S6   ####
...
Temp.ccf <- ccf(as.ts(water.BR2$hourlyTemp), as.ts(water.BR2$Detects2), plot = F) # Significant on the negative, -12 - -34 hours
DO.ccf <- ccf(as.ts(water.BR2$hourlyDO), as.ts(water.BR2$Detects2), plot = F)   # not signif
Flow.ccf <- ccf(as.ts(water.BR2$hourlyFlow), as.ts(water.BR2$Detects2), plot = F) # Significant on the positive, 30 hours and  -12 hours
Gage.ccf <- ccf(as.ts(water.BR2$hourlyGage), as.ts(water.BR2$Detects2), plot = F) # not signif
pH.ccf <- ccf(as.ts(water.BR2$hourlypH), as.ts(water.BR2$Detects2), plot = F)   # Significant on the negative, -35 - -27 hours, -8 - +34 hours

plot(Temp.ccf[Temp.ccf$lag[1:35],])
plot(DO.ccf[DO.ccf$lag[1:35],])
plot(Flow.ccf[Flow.ccf$lag[1:35],])
plot(Gage.ccf[Gage.ccf$lag[1:35],])
plot(pH.ccf[pH.ccf$lag[1:35],])
...
ggsave("Fish13.jpeg", plot = last_plot(), width = 5,height = 4, units = "in",
       dpi = 300,  path = "C:/Users/RER/Documents/Masters UD/Roday et al. 2024 Manuscript/FinalFigs") 
...
# Different
#####
dtc_final2 %>% filter(animal_id == 24) %>% 
  #filter(Year == 2022) %>%
  #filter(Region2 == "Brandywine") %>% 
